...
- Le [RPG](https://geoservices.ign.fr/rpg) recense les parcelles déclarées à la PAC par les agricuteurs : données graphiques et leur culture principale.

  Les données (environ 10 millions de parcelles) sont disponibles dans une base PostgreSQL/PostGIS.
...
Il fournit des paramètres agro-météorologiques de surface quotidiens sur les quarante dernières années à une résolution spatiale de 0,1 ° (soit environ 8×11 km en France). 
...
Estimer l'évolution des dates potentielles de récolte du maïs grain dans les différents bassins de productions français depuis 1980.
...
On peut, par exemple, extraire les principales zones où la densité de culture du maïs grain (code culture : `MIS`) est la plus forte. Cela peut se faire par commune ou avec un lissage.
...
```{r}
#| label: tbl-tendance
#| tbl-cap: !expr "glue('Évolution de la date de récolte : environ {round(mod$coefficients[[\\'annee\\']], 1)} jour par an')"
