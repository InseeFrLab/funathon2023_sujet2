...
$DJ_{n} = \sum_{j=1}^{n}{\frac{T_{j, max} - T_{j, min}}{2} - T_{base}}$
...
-   [ERA5](https://cds.climate.copernicus.eu/cdsapp#!/dataset/sis-agrometeorological-indicators) est un jeu de données qui fournit des données météorologiques de surface quotidiennes pour la période allant de 1979 à aujourd'hui à une résolution spatiale de 0,1 ° (soit environ 10 km en France).  
Les données 2017-2022 ont été préchargées sur Minio et limitées à l'emprise de la métropole. Elle sont au format NetCDF. Un package R existe pour importer et requêter ce jeu de données particulier : [{ag5Tools}](https://github.com/AgrDataSci/ag5Tools).
...
# $ mc cp -r funathon2023_sujet2/donnees/era5/ s3/projet-funathon/2023/sujet2/era5
...
# la date n'est pas un bon indicateur pour suivre une évolution journalière à
# cause des années bissextiles ; on utilisaera le day of year (doy)
# yday(ymd("2020-04-01"))
# yday(ymd("2023-04-01"))
